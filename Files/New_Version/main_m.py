...
def setup_arduino():
    arduino_com = variables["test_parms"]["arduino_com"]
    arduino = Serial(port="COM" + arduino_com, baudrate=115200, timeout=0.1)
...
def setup_board(is_synthetic):
    ...
    timestamp_channel = board.get_timestamp_channel(
        ...
        if not is_synthetic
        else BoardIds.SYNTHETIC_BOARD.value
    )
...
lodb = base_path
folder = (
    f'S{subject_ID}R{repetition_num}_{datetime.datetime.now().strftime("%d%m%Y_%H%M")}'
)
...
df_emotions = pd.DataFrame(
    data=[],
    columns=["Timestamp", "Engagement", "Emotion", "Valence", "Arousal", "Dominance"],
)
...
df_fear_spherical = pd.DataFrame(
    columns=["Timestamp", "Engagement", "Fear_Metric", "Theta"]
)
            ...
            if evaluation_type in ["cubic", "both"]:
                ...
                if final_descion == 0:  # Solo emociones
                    ...
                    cubic_emotion = emocion(
                        vale_cubic, arou_cubic, domi_cubic, emotions, emotions_fear
                    )
                ...
                if final_descion in [0, 2]:  # Guardar en el DataFrame
                    df_cubic.loc[len(df_cubic)] = [
                        ...
                        vale_cubic,
                        arou_cubic,
                        domi_cubic,  # Valores de valencia, arousal y dominancia
                    ]
