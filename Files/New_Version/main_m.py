...
## SEPTEMBER 28 UPDATE ##
# used to be 128, queremos 250 Hz
sampling_rate = variables['test_parms']['sampling_rate']
...
def setup_board(is_synthetic):
    ...
    timestamp_channel = board.get_timestamp_channel(
        BoardIds.CYTON_BOARD.value if not is_synthetic else BoardIds.SYNTHETIC_BOARD.value)
    acc_channel = board.get_accel_channels(
        BoardIds.CYTON_BOARD.value if not is_synthetic else BoardIds.SYNTHETIC_BOARD.value)
...
# CAMBIAR 0.5 POR VALOR NEUTRO DE ENGAGEMENT??
def escalado_11(x): return (x - 0.5) * 2


subject_ID, repetition_num = input(
    'Please enter the subject ID and the number of repetition: ').split(' ')
subject_ID = '0' + subject_ID if int(subject_ID) < 10 else subject_ID
repetition_num = '0' + \
    repetition_num if int(repetition_num) < 10 else repetition_num
lodb = base_path
...
df_cubic = pd.DataFrame(
    columns=[
        ...
        'Arousal',
        'Dominance'])
df_fear_spherical = pd.DataFrame(
    columns=[
        'Timestamp',
        'Engagement',
        'Fear_Metric',
...
if __name__ == "__main__":
    try:
        ...
        data_dict_acc = {
            f'Channel_{channel}': channel_data_acc[i] for i,
            channel in enumerate(acc_channel)}
        ...
        data_dict = {
            f'Channel_{channel}': channel_data[i] for i,
        ...
        df4 = df3[['Channel_1', 'Channel_2', 'Channel_3', 'Channel_4',
                   'Channel_5', 'Channel_6', 'Channel_7', 'Channel_8']].copy()
        ...
        filtered_df = df5.apply(
            lambda col: butter_bandpass_filter(
                col, lowcut, highcut, fs))
        ...
        for column in df_average_reference.columns:
            ...
            psd_bands = compute_psd_bands(
                df_average_reference[column].values, fs=128)
        ...
        # Create a new 'channel_band' column by combining 'channel' and 'index'
        # columns
        melted_df['channel_band'] = melted_df['channel'] + \
            '_' + melted_df['index']

        # Pivot the DataFrame to get the desired format
        new_df = melted_df.pivot(
            index='index',
            columns='channel_band',
            values='value')
        ...
        for channel in CANALES:
            df_pred[f'{channel}_Fatigue'] = df_pred[f'{channel}_Alpha'] / \
                df_pred[f'{channel}_Theta']

        for channel in CANALES:
            df_pred[f'{channel}_Excitement'] = df_pred[f'{channel}_Beta'] / \
                df_pred[f'{channel}_Alpha']

        for channel in CANALES:
            df_pred[f'{channel}_Relaxation'] = df_pred[f'{channel}_Theta'] / \
                df_pred[f'{channel}_Delta']
        ...
        if evaluation_type in ['cubic', 'both']:
            print(
                f"Cubic V{vale_cubic:.2f}, A{arou_cubic:.2f}, D{domi_cubic:.2f}")
        if evaluation_type in ['linear', 'both']:
            print(
                f"Linear V{vale_linear:.2f}, A{arou_linear:.2f}, D{domi_linear:.2f}")
        ...
        # Definicion de cuales funciones se mandan a llamar en base al modelo
        # linear o cubico
        ...
        if evaluation_type in ['cubic', 'both']:
            ...
            if final_descion == 0:  # Solo emociones
                ...
                print(
                    f"Cubic Fear: {cubic_emotion.replace('Fear', '').strip()}")
            elif final_descion == 2:  # Ambos
                cubic_emotion = emocion(
                    vale_cubic,
                    arou_cubic,
                    domi_cubic,
                    emotions,
                ...
                print(
                    f"Cubic Emotion: {emotion_value}, Fear: {fear_value.replace('Fear', '').strip()}")
            ...
            if final_descion in [0, 2]:  # Guardar en el DataFrame
                df_cubic.loc[len(df_cubic)] = [
                    ...
                    # Emoción cúbica o valor emocional
                    cubic_emotion if final_descion == 0 else emotion_value,
                    vale_cubic, arou_cubic, domi_cubic  # Valores de valencia, arousal y dominancia
                ]

        if evaluation_type in ['spherical', 'both']:
            # Valores normalizados para la esfera emocional
            detected_values = np.array([vale, domi, arou]).flatten()
            ...
            if final_descion in [0, 2]:
                try:
                    spherical_emotion, temp, sphere_vector = get_emotion_sphere(
                        detected_values)
            ...
            if final_descion == 0:
                ...
                print(
                    f"Spher Emotion: {spherical_emotion} {temp * 100:.2f}%, Fear: {fear_label} {fear_metric * 100:.2f}%")
         ...
         # Guardar emociones esféricas en el DataFrame
            if final_descion in [0, 2]:
                df_spherical.loc[len(df_spherical)] = [
                    ...
                    # Vector esférico normalizado
                    sphere_vector.tolist() if sphere_vector is not None else None,
                    vale, arou, domi
                ]

        emociones = emocion(arou, domi, vale, emotions,
                            emotions_fear)  # Aqui no se
        # ID categórico de la emoción
        ...
        if final_descion == 0:
            df_emotions.loc[len(df_emotions)] = [
                time.time() - 21600, engag, emociones, vale, arou, domi]
        elif final_descion == 1:
            df_fear.loc[len(df_fear)] = [time.time() - 21600,
                                         engag, emociones, vale, arou, domi]
        elif final_descion == 2:
            df_emotions.loc[len(df_emotions)] = [
                time.time() - 21600, engag, emociones[0], vale, arou, domi]
            df_fear.loc[len(df_fear)] = [time.time() -
                                         21600, engag, emociones[1], vale, arou, domi]
